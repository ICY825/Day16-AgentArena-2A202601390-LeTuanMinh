...
def _quoted_from_one_line(text: str, body: str) -> bool:
...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            current = corpus.get(claim.get("doc_id"))
            if current is not None and _quoted_from_one_line(text, current.body):
                continue
            ...
            source = next(
                (
                    ...
                    if doc.body in observed and _quoted_from_one_line(text, doc.body)
                ),
                None,
            )
